...
#             bat_all = torch.tensor(np.concatenate((all_loc, all_demand), 1), dtype=torch.float)
    ...
    return torch.cat([
        eval_model_bat(bat)
        for bat in tqdm(loader)
    ], 0)


def rollout_groundtruth(problem, dataset, opts):
    return torch.cat([
        problem.get_costs(torch.cat((torch.cat((bat['depot'][:, None], bat['loc']), 1), torch.cat((torch.zeros([bat['demand'].size()[0], 1]), bat['demand']), 1)[:, :, None]), 2), bat['tour_nodes'][:, :-1].type(torch.int64))[0]
...
def train_epoch(model, optimizer, baseline, lr_scheduler, epoch, val_datasets, problem, tb_logger, opts):
    ...
    train_dataloader_upd = dataiter.next()
    ...
    for batch_id, batch in enumerate(tqdm(train_dataloader_upd, disable=opts.no_progress_bar, ascii=True)):
...
#     x = move_to(bat, opts.device)
    ...
    if opts.model == 'nar':
        ...
        _targets = batch['adj'].numpy().flatten()
        ...
        class_weights = None
        targets = move_to(batch['tour_nodes'].long(), opts.device)
